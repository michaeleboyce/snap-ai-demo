...
export default function InterviewProgress({ messages, onCoverageChange }: InterviewProgressProps) {
  ...
  // Memoize sections computation to prevent infinite re-renders
  const sections: InterviewSection[] = useMemo(() => {
    ...
    const baseSections: InterviewSection[] = [
      {
        id: 'household',
    ...
    const householdSection = baseSections.find(s => s.id === 'household');
    if (householdSection) {
    ...
    const incomeSection = baseSections.find(s => s.id === 'income');
    if (incomeSection) {
    ...
    const expenseSection = baseSections.find(s => s.id === 'expenses');
    if (expenseSection) {
    ...
    const assetSection = baseSections.find(s => s.id === 'assets');
    if (assetSection) {
      const assetPatterns = [
    ...
    const specialSection = baseSections.find(s => s.id === 'special');
    if (specialSection) {
    ...
    return baseSections;
  }, [messages, aiSections]); // analyzeContent is defined inside useMemo, so not needed in deps
